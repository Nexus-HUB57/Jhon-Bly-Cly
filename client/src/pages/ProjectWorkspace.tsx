...
export default function ProjectWorkspace() {
  ...
  const requestGeneration = trpc.video.projects.requestVideoGeneration.useMutation({ onSuccess: async () => { toast.success("Geração encaminhada e registrada no orquestrador."); await refresh(); }, onError: error => toast.error(error.message) });
    ...
    <div className="mx-auto max-w-[1500px] space-y-6 pb-10">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button className="bg-slate-950 text-white hover:bg-slate-800" onClick={() => requestGeneration.mutate({ projectId })} disabled={isGenerating || !scenes.length}><Play className="mr-2 h-4 w-4" /> {isGenerating ? "Gerando" : "Gerar vídeo"}</Button>
        </div>
      </header>
      ...
      <Tabs defaultValue="review" className="space-y-5">
        ...
        <TabsContent value="assets" className="mt-0"><div className="grid gap-5 xl:grid-cols-[0.75fr_1.25fr]"><GlassPanel className="p-6"><span className="eyebrow">BIBLIOTECA SEGURA</span><h2 className="mt-2 text-xl font-black">Arquivos de referência</h2><p className="mt-2 text-sm leading-6 text-slate-600">Envie imagens, documentos ou mídia de apoio. O arquivo é armazenado no projeto e pode ser associado à cena ativa.</p><Label htmlFor="asset-file" className="mt-6 flex min-h-36 cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-cyan-300 bg-cyan-50/40 p-5 text-center transition-colors hover:bg-cyan-50"><FileUp className="mb-3 h-6 w-6 text-cyan-700" /><span className="text-sm font-bold text-cyan-800">{isUploading ? "Armazenando ativo…" : "Selecionar arquivo"}</span><small className="mt-1 text-xs text-slate-500">até 10 MB</small><input id="asset-file" type="file" className="sr-only" disabled={isUploading} onChange={event => onUpload(event.target.files?.[0])} /></Label></GlassPanel><GlassPanel className="p-3">{assets.length ? <div className="divide-y divide-slate-100">{assets.map(asset => <div key={asset.id} className="flex items-center gap-3 p-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-slate-100 text-slate-500"><FileUp className="h-4 w-4" /></div><div className="min-w-0 flex-1"><b className="block truncate text-sm">{asset.name}</b><small className="text-xs text-slate-500">{asset.kind} · {asset.mimeType}</small></div><a href={asset.url} target="_blank" rel="noreferrer" className="text-xs font-bold text-cyan-700">Abrir</a></div>)}</div> : <div className="grid min-h-64 place-items-center text-center text-sm text-slate-500">Nenhum ativo associado a este projeto.</div>}</GlassPanel></div></TabsContent>

        <TabsContent value="results" className="mt-0"><ResultsPanel status={project.status as TaskStatus} runs={runs} assets={assets} onExport={() => exportManifest.mutate({ projectId })} exporting={exportManifest.isPending} /></TabsContent>
