...
export default function ProjectWorkspace() {
  ...
  const requestGeneration = trpc.video.projects.requestVideoGeneration.useMutation({ meta: { errorHandling: "local" }, onSuccess: async () => { toast.success("Geração encaminhada e registrada no orquestrador."); await refresh(); }, onError: async error => { toast.error(error.message); await refresh(); } });
  ...
  return (
    <div className="mx-auto max-w-[1500px] space-y-6 pb-10">
      ...
      <Tabs defaultValue="review" className="space-y-5">
        ...
        <TabsContent value="results" className="mt-0"><ResultsPanel status={project.status as TaskStatus} runs={runs} assets={assets} events={events} onExport={() => exportManifest.mutate({ projectId })} exporting={exportManifest.isPending} onPoll={activeVideoRun ? () => pollGeneration.mutate({ projectId, runId: activeVideoRun.id }) : undefined} polling={pollGeneration.isPending} /></TabsContent>
